		...
		if ( empty( $postarr['ID'] ) && ( ! is_multisite() || is_multisite() && ! current_user_can( 'create_users' ) ) ) {
			...
			if ( ! current_user_can( get_option( 'upl_manage_cap' ) ) ) {
				...
				if ( '' !== $relevant_rule ) {
					...
					$prepared_message = str_replace( [ '{extra_posts}', '{limit}', '{count}', '{type}' ],[ $upl_query->found_posts - get_option( 'upl_num_limit' )[ $relevant_rule ] + 1, get_option( 'upl_num_limit' )[ $relevant_rule ], $upl_query->found_posts, get_post_type_object( get_option( 'upl_posts_type' )[ $relevant_rule ] )->labels->name ], do_shortcode( apply_filters( 'upl_message', get_option( 'upl_message' ) ) ), $postarr );
	...
	public function wp_upl_hide_shortcode( $atts, $content = "" ) {
		if ( ! current_user_can( get_option( 'upl_manage_cap' ) ) && ( ! is_multisite() || is_multisite() && ! current_user_can( 'create_users' ) ) ) {
			...
			if ( '' !== $relevant_rule ) {
				...
				$prepared_message = str_replace( [ '{extra_posts}', '{limit}', '{count}', '{type}' ],[ $upl_query->found_posts - get_option( 'upl_num_limit' )[ $relevant_rule ] + 1, get_option( 'upl_num_limit' )[ $relevant_rule ], $upl_query->found_posts, get_post_type_object( get_option( 'upl_posts_type' )[ $relevant_rule ] )->labels->name ], do_shortcode( $atts['message'] ) );
